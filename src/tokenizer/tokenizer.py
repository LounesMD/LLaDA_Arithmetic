...
class naive_pad_tokenizer:
    ...
    def decode(self, token_list):
        try:
            equal_idx = token_list.index(11)
        except ValueError:
            return "".join([self.id_to_token[x] for x in token_list])

        a = token_list[: self.number_bits]
        b = token_list[self.number_bits + 1 : 2 * self.number_bits + 1]
        result = token_list[equal_idx + 1 :]
        ...
        return (
            ...
            + "".join(map(str, b))
            + "="
            + "".join(map(str, [self.id_to_token[x] for x in result]))
        )


class group_pad_tokenizer:
    """
    character-level
    """
    def __init__(
